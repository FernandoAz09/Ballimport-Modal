...
function createModalStyle() {
    ...
    if (!ballImportModalStyleExists) {
        ...
        ballImportModalStyle.innerHTML = `
            #ballImport-Modal.ballImport-Modal__Show .ballImport-Modal__Overlay {
                display: block
            }

            html:has(#ballImport-Modal.ballImport-Modal__Show) {
                overflow-y: hidden;
            }

            div#ballImport-Modal:not(.ballImport-Modal__Show) {
                    display: none !important;
            }

            #ballImport-TableModalLink svg {
                    width: 20px;
                    height: 20px;
                    position: absolute;
                    top: 8px;
                    fill: rgb(252 87 31) !important;
                    color: rgb(252 87 31) !important;
            }

            #ballImport-TableModalLink {
                    position: relative;
                    padding: 10px 0;
                    cursor: pointer;
            ...
            #ballImport-TableModalLink a {
                    margin-left: 25px;
                    color: rgb(252 87 31) !important;
                    cursor: pointer;
                    text-decoration: none;
            }
            #ballImport-Modal.ballImport-Modal__Show .ballImport-Modal__Container .ballImport-Modal__ContainerWrapper {
                    opacity: 1;
                    visibility: visible
            }

            #ballImport-Modal .ballImport-Modal__Overlay {
                    background: #7b7b7b;
                    border-color: #7b7b7b;
                    opacity: .8;
                    height: 100%;
            ...
            #ballImport-Modal .ballImport-Modal__Container .ballImport-Modal__ContainerWrapper {
                    position: fixed;
                    z-index: 2000;
            }

            #ballImport-Modal .ballImport-Modal__Container .ballImport-Modal__ContainerWrapper.ballImport-Modal__HasShadow {
                    box-shadow: 0 4px 8px 0 rgba(0, 0, 0, .2), 0 6px 20px 0 rgba(0, 0, 0, .19)
            }

            #ballImport-Modal .ballImport-Modal__Container .ballImport-Modal__ContainerWrapper header {
            ...
            #ballImport-Modal .ballImport-Modal__Container .ballImport-Modal__ContainerWrapper .ballImport-Modal__Content {
                    padding: 15px;
                    overflow-y: auto;
                    display: flex;
            ...
            #ballImport-Modal .ballImport-Modal__Container .ballImport-Modal__ContainerWrapper button.ballImport-Modal__Close {
                    cursor: pointer;
            ...
            @media only screen and (max-width: 749px) {
                    ...
                    #ballImport-Modal .ballImport-Modal__Container .ballImport-Modal__ContainerWrapper .ballImport-Modal__Content img {
                            width: 100%;
                    }
...
function createModalLink() {
    ...
    if (!linkExistente) {
        ...
        var seletorElemento = document.querySelector('section.main-product-content.container .product-customizations');
        seletorElemento.prepend(ballImportModalLink);
    }
}
...
function createModal() {
    if (!document.getElementById('ballImport-Modal')) {
        ...
        ballImportModal.innerHTML = `
            <div class="ballImport-Modal__Overlay" style="background: rgb(123, 123, 123); border-color: rgb(123, 123, 123);"></div>
            <div class="ballImport-Modal__Container">
                <div class="ballImport-Modal__ContainerWrapper ballImport-Modal__Full ballImport-Modal__HasShadow"
                    style="background: rgb(255, 255, 255); border-color: rgb(0, 0, 0); color: rgb(0, 0, 0); border-radius: 3px; height: 95%; width: 95%; top: 50%; left: 50%; transform: translate(-50%, -50%);">
                    <header>Tabela de Medidas</header>
                    <div id="ballImport-modal-content" class="ballImport-Modal__Content" style="height: calc(100% - 95px);">
                        <div>
                            <p>
                                <img src="https://i.ibb.co/CPB27td/tabela-de-medidas-ball-2023-6.png" alt="Uploaded Image"
                                    style="display: block; margin-left: auto; margin-right: auto;">
                            </p>
                        </div>
                    </div>
                    <button title="Close (Esc)" type="button" class="ballImport-Modal__Close"
                        style="color: rgb(0, 0, 0);">x</button>
                </div>
            </div>
        `;
